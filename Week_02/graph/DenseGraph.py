...
class DenseGraph:
    ...
    def __init__(self, num_of_vertex, directed):
        ...
        self.info = [[None for _ in range(self.v)] for _ in range(self.v)]  # 记录图的具体数据， 使用v*v大小的布尔型矩阵来表示
    ...
    def add_edge(self, n, m, weight):
        ...
        if self.has_edge(n, m):  # 已经存在边就进行删除
            self.info[n][m] = None
            if not self.directed:
                self.info[n][m] = None
            self.e -= 1

        self.info[n][m] = Edge(n, m, weight)
        if not self.directed:
            self.info[m][n] = Edge(m, n, weight)
        self.e += 1
    ...
    def show(self):
        for i in range(self.v):
            ...
            for j in range(self.v):
                if self.info[i][j] is None:
                    print(None, end=' ')
                else:
                    print(self.info[i][j].weight_, end=' ')
            print()
        ...
        class EdgeIterator:
            ...
            def next(self):
                ...
                for _ in range(self.index, self.graph.num_of_vertex):
                    if graph.info[self.v][self.index] is not None:
                        return graph.info[self.v][self.index]
                    self.index += 1
                return None  # 找不到相邻顶点时返回-1
